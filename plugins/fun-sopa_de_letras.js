...
await conn.reply(m.chat, `*@${m.sender.split("@")[0]} ${await tr("REGISTRADO EN EL JUEGO")}* ✅`, m, { mentions: [m.sender] })
}
...
await m.reply(`🔠 ${await tr("*SOPA DE LETRAS*")} 🔠
*${await tr("PALABRA")}:* \`\`\`"${PALABRA}"\`\`\`
*${await tr("TIENE 3 MINUTOS PARA ENCONTRAR LA RESPUESTA CORRECTA!!")}*

*${await tr("ESCRIBA EL NÚMERO DE FILA Y COLUMNA DEL COMIENZO DE LA PRIMERA LETRA")} _"${PALABRA.charAt(0)}"_ ${await tr("DE LA PALABRA")} _"${PALABRA}"_ ${await tr("TIENE")} _${intentos}_ ${await tr("INTENTOS!!")}*
...
await conn.reply(m.chat, `*@${tagUser.split("@")[0]} ${await tr("ESTA JUGANDO SOPA DE LETRAS 🔠 ACTUALEMENTE")}*`, m, { mentions: [tagUser] })
return
}
if (intentos === 0) {
intentos = 3  
...
if (intentos !== 0) {
await conn.reply(m.chat, `*@${m.sender.split("@")[0]} ${await tr("TE QUEDA UN MINUTO!!")}* 😨`, m, { mentions: [m.sender] })
}
await new Promise((resolve) => setTimeout(resolve, 3 * 60 * 1000)) // 3 min
if (intentos !== 0) {
await conn.reply( m.chat, `*@${m.sender.split("@")[0]} ${await tr(`EL TIEMPO SE HA ACABADO!!* 😧\n\nLA PALABRA_ "${sopaPalabra}"_ SE ENCONTRABA EN LA DIRECCIÓN _${sopaDir}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_`)} `, m, { mentions: [m.sender] })
...
await m.reply(`\`\`\`🎊 ${await tr("HAS GANADO")} ${diamante} ${rpgshop.emoticon('limit')}!!\`\`\`\n\n*${await tr("CORRECTO!! LA PALABRA")} _"${sopaPalabra}"_ ${await tr(`SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_`)}*`)
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
return
}else{
...
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
await m.reply(`🫡 *${await tr("AGOTASTE LOS INTENTOS!! LA PALABRA")} _"${sopaPalabra}"_ ${await tr(`SE ENCONTRABA EN LA DIRECCIÓN _${cambioLetra}_ DE LA FILA _${fila}_ Y COLUMNA _${columna}_`)}*`)
return  
} else {
intentos -= 1
await m.reply(`😮‍💨 *${await tr(`INCORRECTO. TE QUEDAN _${intentos}_ INTENTOS!!`)} *${intentos === 1 ? '' : `\n*${await tr("PALABRA A ENCONTRAR")}:* \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`💡 ${await tr("PISTA!!")}\`\`\`\n*${await tr("LA PALABRA")} _${sopaPalabra}_ ${await tr("SE ENCUENTRA EN LA DIRECCIÓN")} _"${cambioLetra}"_*\n\n` : ''}${sopaNube}`)
return
}}
}}

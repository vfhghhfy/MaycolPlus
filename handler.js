        ...
        try {
            ...
            if (user) {
                ...
                global.db.data.users[m.sender] = {
                    ...
                    regTime: -1, afk: -1, afkReason: '', banned: false, useDocument: false,
        ...
        for (let name in global.plugins) {
            ...
            if ((usedPrefix = (match[0] || '')[0])) {
                ...
                let extra = {
                    ...
                    conn: this, participants, groupMetadata, user, channel,
                    bot, isROwner, isOwner, isRAdmin, isAdmin, isBotAdmin,
        ...
        if (m) { 
            ...
            if (m.plugin) {
                ...
                if (m.plugin in stats) {
                    ...
                    stat = stats[m.plugin] = {
                        total: 1, success: m.error != null ? 0 : 1, last: now,
...
global.dfail = (type, m, usedPrefix, command, conn) => {
    ...
    const msg = {
      rowner: `
╔═══════❖『 ✦ ${global.botname} ✦ 』❖═══════╗
║ (≖ᴗ≖✿) El hechizo *${command}* solo puede ser
║ invocado por los Dioses del retrete (creadores).
╚════════════════════════════════════╝`,
    
      owner: `
╭─┈ ✦『 Control Paranormal 』✦ ┈─╮
│ ⛧ *${command}* está reservado para los  
...
║ pueden controlar el hechizo *${command}*.  
╚════════════════════════════════════╝`,
    
      premium: `
╭───✿『 ✦ ✧ MOCHI PREMIUM ✧ ✦ 』✿───╮
│ (◍•ᴗ•◍)❤ Solo almas elegidas pueden usar  
│ el comando sagrado *${command}*.  
╰──────────────────────────────────╯`,
    
      group: `
╔═━「 ☁︎ Ritual Grupal ☁︎ 」━═╗
║ (⁎⁍̴̛ᴗ⁍̴̛⁎) *${command}* requiere un círculo  
║ de invocación múltiple (grupo).  
╚═════════════════════════════╝`,
    
      private: `
╭──⊹⊱『 ☁︎ Susurro Privado ☁︎ 』⊰⊹──╮
│ Este hechizo *${command}* solo puede ser  
│ revelado en un santuario secreto (chat privado).  
╰─────────────────────────────────────╯`,
    
      admin: `
╭────『 ✦ Líder Espiritual ✦ 』────╮
│ El comando *${command}* sólo responde a  
│ los shamanes del grupo (admins).  
╰─────────────────────────────╯`,
    
      botAdmin: `
(╥﹏╥) ¡Ay no!  
No puedo usar *${command}* si no soy un  
espíritu superior (admin del grupo).  
¡Dame poder MUAJAJAJA o me encierro en el baño para siempre >:(`,
    
      unreg: `
╔══『 ✦ PACTO FANTASMAL ✦ 』══╗
║ (｡•́︿•̀｡) No puedes usar *${command}* ║ hasta que firmes tu contrato espiritual.
║ Usa el ritual:
...
╚═══════════════════════════╝`,
      ...
      channel: `
╔══『 ✦ SEGUDORES FANTASMAS ✦ 』══╗
║ (｡•́︿•̀｡) No puedes usar *${command}* ║ Solo se permite en canales.
